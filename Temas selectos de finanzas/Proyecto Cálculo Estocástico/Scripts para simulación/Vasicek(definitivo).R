...
#Función que calcula los estimadores de máxima verosimilitud
VAS.MLE<-function(r,dt){
          n<-length(r)
          u<-r[1:(n-1)]  
          v<-r[2:n]
          alpha <- ((n-1)*sum(u*v) -sum(u)*sum(v))/((n-1)*sum(u^2) - sum(u)^2) 
          beta <- sum(v - alpha*u)/((n-1)*(1-alpha))
          V.2<-(1/(n-1))*sum((v - alpha*u - beta*(1-alpha))^2)
          a<--(1/dt)*log(alpha)
          return(c(alpha,beta,sqrt((2*a*V.2)/(1-exp(-2*a*dt))))) 
        }
##Vasicek 
VAS<-function(a,r_0,n,dt){   
         r<-numeric(n)
         r[1]<-r_0
         z<-rnorm(n-1) 
         sapply(1:(n-1),function(i)r[i+1]<<-r[i]*a[1] + a[2]*(1 - a[1]) + a[3]*sqrt((1-(a[1])^2)/(-2*(1/dt)*log(a[1])))*z[i])
         return(r)
         }

##Simulación Vasicek 
VAS.SIM <- function(r,date,tenor = True,deltat = 1/50,N = 50,seed = 46,nsims = 50000,conf = c(.005,.995),n.tr = 20){ 
 ifelse(tenor,r<-r,r<-log(1+r))
 v<-tail(date,n = 1) 
 Y<-as.Date(sapply(seq(0,(N-1)*7,7),function(x)v + x),origin = "1970-01-01")
 a<-VAS.MLE(r,dt = deltat)
 r_0<-r[length(r)] 
 
 set.seed(seed)
 d<-sapply(rep(N,nsims),function(x)VAS(a=a,r_0=r_0,n=x,dt=deltat)) 
 ...
 par(mfrow = c(1,3))                        
   ...
   plot(date,r,col = "#99AEB6",main="Tasas CETES 360",
     ...
     box(lwd=2) 
   plot(Y,d[,samp],col = "red",main="Simulación tasas CETES 360 con modelo Vasicek \n(Una Trayectoria)",
     xlab = "Fechas",ylab = "Nivel de la tasa", type = "l",lwd=2)
     box(lwd=2)
   plot(Y,s.3,col = "red",main="Media e intervalo de confianza + trayectorias",
     xlab = "Fechas",ylab = "Nivel de la tasa", type = "l", 
      ylim=c(min(s.3),max(s.1)),lty = "dashed",lwd=2)
      sapply(1:n.tr,function(i)lines(Y,d[,Samp[i]],col = blues[i],lwd =2))
      lines(Y,s.1,col = "red",lty = "dashed",lwd = 2)
      lines(Y,s,col = "black",lty = "dotdash",lwd = 3)
     box(lwd=2)
}

##Tarda aproximadamente 6.17 segundos en correr (Tasa con cap continua)
##Se recomienda ampliar la ventana de gráficos para una mejor visualización
system.time(
VAS.SIM(r,y,tenor = 0)
)
...
VAS.SIM.M <- function(r,date,tenor = True,deltat = 1/50,N = 50,nsims = 50000,conf = .995,n.tr = 8,color = "p"){ 
 ifelse(tenor,r<-r,r<-log(1+r))
 v<-tail(date,n = 1) 
 Y<-as.Date(sapply(seq(0,(N-1)*7,7),function(x)v + x),origin = "1970-01-01")
 a<-VAS.MLE(r,dt = deltat)
 ...
 y<-c(y,Y)
 co<-numeric(1)
 ifelse(color =="p",co<-"#D219FF",co<-"blue") 
 blues<-c("#D4FDFF","#28F7FF","#42DEFF","#00CBFF","#00B7E6","#00A8D3","#008BD3","#007BBA","#0038AB","#6600D8")
 blues<-rep(blues,30)
 greens<-c("#E2CCFB","#D2B6F2","#C7A5EE","#BC96E7","#B387E5","#AC7BE3","#9F68DC","#9254D7","#7C3EC1","#6026A1")
 greens<-rep(greens,30)
 ifelse(color == "p",color<-greens,color<-blues) 
 par(mfrow = c(1,1))
 plot(y,M,col = "blue",main=paste("Tasas CETES 360 y posibles escenarios (Vasicek) \n+ intervalos de confianza al ",as.character(conf*100),"%"),
...
system.time(
a<-VAS.SIM.M(r,y,tenor = 0,nsims =10000,n.tr = 9)
)
##confianza 95%
system.time(
VAS.SIM.M(r,y,tenor = 0,nsims =10000,n.tr = 9,conf = .95)
)
...
system.time(
a<-VAS.SIM.M(r,y,tenor = 1,nsims =10000,n.tr = 9,col = "b")
)
##confianza 95%
system.time(
VAS.SIM.M(r,y,tenor = 1,nsims =10000,n.tr = 9,col = "b",conf = .95)
)
